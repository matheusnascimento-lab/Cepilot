...
export default function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(hasOnboarding ? '/tutor' : '/onboarding')
    } catch {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
      ...
      <div className="content" style={{ width: '100%', maxWidth: '420px', padding: '1.5rem' }}>
        {/* Logo */}
        <div className={`fade-up ${mounted ? '' : 'opacity-0'}`} style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          <div style={{
            display: 'inline-flex',
            alignItems: 'center',
            gap: '10px',
            ...
            <span style={{ fontSize: '1.4rem', fontWeight: '700', letterSpacing: '-0.02em', color: '#F0EEFF' }}>
              Cepi<span className="gradient-text">lot</span>
            </span>
          </div>

          <h1 style={{ fontSize: '2rem', fontWeight: '700', letterSpacing: '-0.03em', marginBottom: '0.5rem', color: '#F0EEFF', lineHeight: '1.2' }}>
            Seu copiloto<br />
            <span className="gradient-text-pink">de aprendizado</span>
          </h1>
          <p style={{ fontSize: '0.9rem', color: 'var(--muted)', lineHeight: '1.6' }}>
            Entre com sua conta CEFIS e decole
          </p>
        </div>
        ...
        <div className={`card fade-up delay-2`} style={{ padding: '2rem' }}>
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: '500', marginBottom: '0.5rem', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                E-mail ou CPF
              </label>
              <input
                type="text"
            ...
            <button
              ...
              style={{ width: '100%', fontSize: '0.95rem', padding: '0.875rem', marginTop: '0.25rem' }}>
              {loading ? (
        ...
        {/* Bottom note */}
        <div className="fade-up delay-4" style={{ textAlign: 'center', marginTop: '1.25rem' }}>
          <p style={{ fontSize: '0.75rem', color: 'rgba(123,116,148,0.6)' }}>
            Hackathon CEFIS 2026 · Powered by OpenAI
          </p>
        </div>
      </div>
